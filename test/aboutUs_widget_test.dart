...
testWidgets('Checks each FeatureLine for correct icon and text', (WidgetTester tester) async {
  ...
  void testFeatureLine(String iconPath, String featureText) {
    ...
    expect(find.byWidgetPredicate(
      (Widget widget) => widget is Image && widget.image is AssetImage && (widget.image as AssetImage).assetName == iconPath
    ), findsOneWidget);
  ...
  testFeatureLine('assets/lock.png', 'Secure storage of medical history, test reports and prescriptions.');
  testFeatureLine('assets/profile.png', 'Patients can register themselves easily and make their profile.');
  testFeatureLine('assets/calendar.png', 'Customizable medication reminders');
  testFeatureLine('assets/code.png', 'Streamlined doctor access to patient records');
});
